...
def export_snapshot(connection, run_id, emit):
    manifest = json.loads(
        (
            ...
            / "data/question_quality_flagged_ids.json"
        ).read_text()
    )
    ids = manifest["ids"]
    if (
        manifest.get("complete") is not True
        or manifest.get("scope") != "v2_publicada"
        or not manifest.get("source_run_id")
        or not isinstance(manifest.get("scanned"), int)
        or manifest["scanned"] <= 0
    ...
    query = text("""SELECT id, source_id, catalog_version, status, enunciado,
        ...
        FROM exam_questions WHERE id IN :ids ORDER BY id""").bindparams(
        bindparam("ids", expanding=True)
    )
    seen = []
    for offset in range(0, len(ids), 100):
        rows = (
            connection.execute(query, {"ids": ids[offset : offset + 100]})
            .mappings()
            .all()
        )
        for row in rows:
    ...
    emit(
        "QUESTION_QUALITY_SNAPSHOT_DONE "
        + json.dumps(
            {
                "run_id": run_id,
                "requested": len(ids),
